...
$pageWidth = $width * $scaleFactor;
$pageHeight = $height * $scaleFactor;
...
/csquare {
        newpath
        0    0   moveto
        0    $scaleFactor lineto
        $scaleFactor  $scaleFactor lineto
        $scaleFactor  0   lineto
        closepath
        setrgbcolor
        fill
} def
...
for( $y=0; $y<$height; $y++ ) {
	for( $x=0; $x<$width; $x++ ) {
		...
		my $pageY = $pageHeight - $scaleFactor * ( $y + 1 );
		
		#$pageY *= $scaleFactor;

		my $pageX = $x * $scaleFactor;
