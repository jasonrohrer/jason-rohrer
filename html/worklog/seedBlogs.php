...
if( ! $justLoggedOut &&
    ...
    if( strcmp( $loggedInID, "" ) != 0 ) { 
        ...
        refreshCookie( $cookie_user_id, $cookie_session_id );
        }
    }
...
?>
    ...
    <TR><TD VALIGN=TOP>
       [<A HREF="seedBlogs.php?action=show_register_form">New Account</A>]</TD>
...
<?php
        ...
        echo "Logged in as <B>" . stripMagicQuotes( $loggedInID ) .
            "</B><BR>\n";
...
function seedBlogs_showSearchBox( $inFieldWidth = 15, $inShowButton = true ) {
    ...
    if( isset( $_REQUEST[ "key_words" ] ) ) {
        $key_words = stripMagicQuotes( getRequestVariableRaw( "key_words" ) );
        }
?>
    <FORM ACTION="seedBlogs.php" METHOD="post">
...
function seedBlogFormatted( $inBlogName,
    ...
    if( strcmp( $loggedInID, "" ) != 0 ) {
        if( $autoApprovePosts ||
            getUserDatabaseField( $loggedInID, "administrator" ) == 1 ) {
            // post, don't submit
    ...
    for( $i=0; $i<$numRows; $i++ ) {
        ...
        if( $mapArray == NULL ) {
            ...
            connectToDatabase();
            $singleResult = queryDatabase( $query );
            closeDatabase();
        ...
        if( $inShowIntroText ) {
            ...
            // link around subject
            echo "$inHeadlineFormatOpen".
                "<A HREF=\"$linkTarget\">$subject_line</A>";

            if( $directURLTarget && canEdit( $post_id ) ) {
...
if( isset( $_REQUEST[ "action" ] ) ) {
    $action = getRequestVariableSafe( "action" );
    }
$post_id = "";
if( isset( $_REQUEST[ "post_id" ] ) ) {
    $post_id = getRequestVariableSafe( "post_id" );
    }
$blog_name = "";
if( isset( $_REQUEST[ "blog_name" ] ) ) {
    $blog_name = getRequestVariableSafe( "blog_name" );
    }

global $return_url;
$return_url = "";
if( isset( $_REQUEST[ "return_url" ] ) ) {
    $return_url = getRequestVariableSafe( "return_url" );
    }
...
if( strcmp( $action, "login" ) == 0 ) {
    ...
    $exists = doesTableExist( $tableName );
    closeDatabase();
...
function login() {
    ...
    $user_id = getRequestVariableSafe( "user_id" );
    // never used in database query, so strip once here
    $password = stripMagicQuotes( getRequestVariableRaw( "password" ) );

    if( doesUserExist( $user_id ) ) {
        
        if( getUserDatabaseField( $user_id, "approved" ) == 0 ) {
            // display failure page
            messagePage( "User ID <B>" .
                         stripMagicQuotes( $user_id ) .
                         "</B> has no been approved yet." );
            }
        else {
            
            $passwordMD5 = computePasswordHash( stripMagicQuotes( $user_id ),
                                                $password );
        
            $truePasswordMD5 = getUserDatabaseField( $user_id,
                                                     "password_md5" );

            if( strcmp( $truePasswordMD5, $passwordMD5 ) == 0 ) {
                
                $session_id = computeSessionID( stripMagicQuotes( $user_id ),
                                                $password );
                
                setUserDatabaseField( $user_id, "session_id", $session_id );
                
                // set cookies with the user_id and session_id
                refreshCookie( $user_id, $session_id );
                
                // set global
                global $loggedInID;
                $loggedInID = $user_id;
                
                // show main page as logged in user
                global $mainSiteURL;
                // redirect
                header( "Location: $mainSiteURL" );
                }
...
function logout() {
    ...
    setUserDatabaseField( $loggedInID, "session_id", NULL );
    ...
    messagePage( "You have successfully logged out." );
    }
 ...
 */
function showRegisterForm( $inMessage ) {
    global $header, $footer;
 ...
 * @param inMessage the message to display.
 */
function showPasswordHelpForm( $inMessage ) {
    global $header, $footer;
...
function sendPasswordEmail() {
    ...
    $email = getRequestVariableSafe( "email" );
    ...
    if( strcmp( $user_id, "" ) == 0 && strcmp( $email, "" ) == 0 ) {
        $error = 1;
        showPasswordHelpForm( "You must provide some account information." );
        }
    ...
    if( ! $error ) {
        ...
        if( strcmp( $user_id, "" ) != 0 ) {
            ...
            $query = "SELECT * FROM $tableNamePrefix"."users ".
                "WHERE email = '$email' AND session_id IS NULL;";
            }
        ...
        if( $numRows == 0 ) {
            ...
            $temp_session_id = computeSessionID( $user_id, $password_md5 );
            ...
            $temp_password_md5 =
                computePasswordHash( stripMagicQuotes( $user_id ),
                                     $temp_password );
            setUserDatabaseField( $user_id,
...
function register() {
    ...
    // never used in database query, so strip once here
    $password = stripMagicQuotes( getRequestVariableRaw( "password" ) );
    $password_b = stripMagicQuotes( getRequestVariableRaw( "password_b" ) );
    ...
    if( !$updateExisting && strcmp( $user_id, "" ) == 0 ) {
        $error = 1;
        showRegisterForm( "\"User ID\" is a required field." );
        }
    else if( !$updateExisting && strcmp( $password, "" ) == 0 ) {
        $error = 1;
        showRegisterForm( "You must enter a password." );
        }
    else if( strcmp( $email, "" ) == 0 ) {
        $error = 1;
        showRegisterForm( "You must enter an email address." );
        } 
    else if( strcmp( $password, $password_b ) != 0 ) {
        $error = 1;
        showRegisterForm( "Your re-typed password does not match." );
        }

    if( ! $error ) {
        if( !$updateExisting && doesUserExist( $user_id ) ) {

            showRegisterForm( "User id <B>$user_id</B> already exists." );
            }
        else if( !$updateExisting && strcmp( $user_id, "Anonymous" ) == 0 ) {

            showRegisterForm( "User id <B>Anonymous</B> is reserved." );
            }
        else if( !$updateExisting ) {
            $password_md5 = computePasswordHash( stripMagicQuotes( $user_id ),
                                                 $password );
            ...
            if( $approved ) {
                ...
                messagePage( "Your account request has been sent to the ".
            ...
            if( strcmp( $password, "" ) != 0 ) {
                ...
                $password_md5 = computePasswordHash(
                    stripMagicQuotes( $loggedInID ), $password );
            ...
            if( strcmp( $password, "" ) != 0 ) {
                ...
                messagePage( "Your account information has been updated.<BR> ".
 ...
 *   show a blank form.
 */
function showEditor( $inBlogName, $inPostID ) {
    global $tableNamePrefix, $autoApprovePosts;

    $show_author = getRequestVariableSafe( "show_author" );
    $show_date = getRequestVariableSafe( "show_date" );
    ...
    if( $inPostID != NULL ) {
        ...
        if( mysql_numrows( $result ) != 1 ) {
            closeDatabase();
            fatalError( "Post $inPostID does not exist in database." );
            }
...
<?php
    ...
    printDateTimeFormFromTimestamp( "expire_",
 ...
 *   insert a new post.
 */
function updatePost( $inBlogName, $inPostID ) {
    global $return_url, $tableNamePrefix, $loggedInID;
    
    $author_name = getRequestVariableSafe( "author_name" );
    $subject_line = getRequestVariableSafe( "subject_line" );

    $return_url = getRequestVariableSafe( "return_url" );
    
    // will encode illegal HTML tags whenver we display the post
    $intro_text = getRequestVariableRaw( "intro_text" );
    $body_text = getRequestVariableRaw( "body_text" );

    $expire_month = getRequestVariableSafe( "expire_month" );
    $expire_day = getRequestVariableSafe( "expire_day" );
    $expire_year = getRequestVariableSafe( "expire_year" );
    $expire_hour = getRequestVariableSafe( "expire_hour" );
    $expire_minute = getRequestVariableSafe( "expire_minute" );
    $expire_ampm = getRequestVariableSafe( "expire_ampm" );
    $never_expires = getRequestVariableSafe( "never_expires" );
    ...
    if( isset( $_REQUEST[ "allow_comments" ] ) ) {
        $allow_comments = getRequestVariableSafe( "allow_comments" );
        }
    if( isset( $_REQUEST[ "show_permalink" ] ) ) {
        $show_permalink = getRequestVariableSafe( "show_permalink" );
        }
    if( isset( $_REQUEST[ "approve" ] ) ) {
        $approve = getRequestVariableSafe( "approve" );
        }
    if( isset( $_REQUEST[ "remove" ] ) ) {
        $remove = getRequestVariableSafe( "remove" );
        }
    ...
    if( $never_expires != 1 ) {
        ...
        $expiration_date = formatTime( $expire_year,
                                       $expire_month,
                                       $expire_day,
                                       $expire_hour,
                                       $expire_minute,
    ...
    */
    ...
    if( $post_id == NULL ) {
        if( strcmp( $loggedInID, "" ) == 0 &&
            ...
            // display failure page
            messagePage( "You must log in to submit posts." );
            }
        else if( strcmp( $author_name, "" ) != 0 &&
                 doesUserExist( $author_name ) ) {
            // a public (not logged in) user has specified an
            // existing user's ID as their author name

            $postAllowed = false;
            messagePage( "The name <B>$author_name</B> is already in".
                         " use by a registered user." );
            }
        else {
            $post_id = getUniquePostID();

            global $autoApprovePosts, $loggedInID;

            if( strcmp( $loggedInID, "" ) != 0 ) {
                if( getUserDatabaseField( $loggedInID,
                                          "administrator" ) == 1 ) {
        ...
        if( !canEdit( $post_id ) ) {
            ...
            // display failure page
            messagePage( "You are not allowed to edit this post." );
            }
        else {
    ...
    if( $postAllowed ) {
        ...
        if( $remove != 1 ) {
            if( $postApproved == 1 ||
                ...
                messagePage( "The post has been submitted for approval." );
                }
            }
        else {
            // let the user know the post was removed
            messagePage( "The post has been removed." );
            }
        }
    }
 ...
 */
function moveUp( $inBlogName, $inPostID ) {
    movePost( $inBlogName, $inPostID, -1 );
    }
 ...
 */
function moveDown( $inBlogName, $inPostID ) {
    movePost( $inBlogName, $inPostID, 1 );
    }
 ...
 */
function movePost( $inBlogName, $inPostID, $inMoveDirection ) {
    global $return_url, $tableNamePrefix, $loggedInID;
    // update the map
    // lock to ensure our update is atomic
    $mapQuery = "SELECT map FROM $tableNamePrefix"."order_map ".
    ...
    if( mysql_numrows( $result ) == 1 ) {
        ...
        if( $postIndex == -1 ) {
            fatalError( "Could not find post $inPostID in $inBlogName " .
                        "order map." );
            }
        ...
        while( ! $doneMoving ) {
            ...
            $nextVisible = isPostVisible( $nextHigherID ); 
...
function displayPost( $inPostID ) {
    ...
    $commentCount = countComments( $inPostID, 1 );
 ...
 */
function showArchive( $inBlogName ) {

    $offset = getRequestVariableSafe( "offset" );
    $count = getRequestVariableSafe( "count" );
    $order = getRequestVariableSafe( "order" );
    $show_intro = getRequestVariableSafe( "show_intro" );
    $show_authors = getRequestVariableSafe( "show_authors" );
    $show_dates = getRequestVariableSafe( "show_dates" );
    $show_submit_link_to_public =
        getRequestVariableSafe( "show_submit_link_to_public" );
...
function approvePost( $inPostID ) {
    ...
    if( $post_id == NULL ) {
        ...
        // display failure page
        messagePage( "No post_id field given." );
        }
    else {
        ...
        if( !isAdministrator() ) {
            ...
            // display failure page
            messagePage( "You must be an administrator to approve posts." );
            }
        else {
            $query = "UPDATE $tableNamePrefix"."posts SET " .
                "approved = '1' " .
...
function approveAccount() {
    ...
    $user_id = getRequestVariableSafe( "user_id" );
    $admin = getRequestVariableSafe( "admin" );
    ...
    if( $user_id == NULL ) {
        ...
        // display failure page
        messagePage( "No user_id field given." );
        }
    else {
        ...
        if( !isAdministrator() ) {
            ...
            // display failure page
            messagePage( "You must be an administrator to approve accounts." );
            }
        else {
            $adminClause = "";
    ...
    if( $approvalAllowed ) {
        ...
        $email = getUserDatabaseField( $user_id, "email" );
...
function changeAdminStatus() {
    ...
    if( $user_id == NULL || $admin == NULL ) {
        ...
        // display failure page
        messagePage( "Required fields are missing." );
        }
    else {
        ...
        if( !isAdministrator() ) {
            ...
            // display failure page
            messagePage( "You must be an administrator to change accounts." );
            }
        else {
...
function removeAccount() {
    ...
    if( $user_id == NULL ) {
        ...
        if( !isAdministrator() ) {
            ...
            // display failure page
            messagePage( "You must be an administrator to remove accounts." );
            }
        else {            
 ...
 *
 * @param $inBlogName the name of the blog to show a queue for. 
 */
function showPostQueue( $inBlogName ) {
    global $header, $footer, $tableNamePrefix;

    if( ! isAdministrator() ) {
        messagePage( "You must be an administrator to view the queue." );
        return;
        }
    ...
    echo "<FONT SIZE=4>Posts from <EM>$inBlogName</EM> ".
    ...
    for( $i=0; $i<$numRows; $i++ ) {
        ...
        echo "<TR><TD "; printNextBGColor(); echo ">$author</TD>";
        echo "<TD "; printNextBGColor(); echo ">$subject_line</TD>";
        
        echo "<TD NOWRAP "; printNextBGColor();
...
function showAccountQueue() {
    ...
    for( $i=0; $i<$numRows; $i++ ) {
        ...
        echo "<TR><TD "; printNextBGColor(); echo ">$user_id</TD>";
        echo "<TD "; printNextBGColor(); echo ">$email</TD>";
...
function showAccountList() {
    ...
    if( ! isAdministrator() ) {
        messagePage( "You must be an administrator to ".
    ...
    for( $i=0; $i<$numRows; $i++ ) {
        ...
        if( $administrator ) {
            echo "<TD "; printNextBGColor(); echo ">admin</TD>";
            }
        else {
            echo "<TD "; printNextBGColor(); echo "></TD>";
            }
...
function search() {
    ...
    $key_words = getRequestVariableSafe( "key_words" );
 ...
 * rendering code so that seedBlogFormatted() and search() can both use it.
 *
 ...
 * seedBlogFormatted.
 */
function generateStoryBlock( $inBlogName,
                             $inPostID,
                             $inSubjectLine,
                             $inUserID,
                             $inDateString,
    ...
    if( canEdit( $inPostID ) ) {
        ...
        if( $inShowDownWidget ) {
            echo "[<A HREF=\"seedBlogs.php?action=move_down".
                "&blog_name=$inBlogName".
                "&post_id=$inReturnURL&return_url=$inReturnURL\">" .
                "Move Down</A>]";
            }
        }
    if( $inIntroText != NULL ) {
        // intro text
        $formattedIntro = rcb_blog2html( $inIntroText );
        echo "<BR>$formattedIntro";
        }
    if( $inBodyText != NULL && $inEmbedBodyText ) {
         $formattedBody = rcb_blog2html( $inBodyText );
    ...
    if( $inShowPermalink ||
        ...
        if( $inAllowComments ) {
            $approvedCount = countComments( $inPostID, 1 );
            $queuedCount = countComments( $inPostID, 0 );
            ...
            if( $approvedCount > 0 ||
                ...
                global $autoApproveComments, $loggedInID;
...
function rssFeed() {
    ...
    $blog_name = getRequestVariableSafe( "blog_name" );
    $channel_title =
        stripMagicQuotes( getRequestVariableSafe( "channel_title" ) );
    $channel_description =
        stripMagicQuotes( getRequestVariableSafe( "channel_description" ) );
    $max_number = getRequestVariableSafe( "max_number" );
    $show_authors = getRequestVariableSafe( "show_authors" );
    $show_dates = getRequestVariableSafe( "show_dates" );
...
<?php
    ...
    for( $i=0; $i<$numRows; $i++ ) {
        ...
        $intro_text =
            htmlspecialchars( rcb_blog2html( strip_tags( $intro_text ) ) );
 ...
 */
function countComments( $inPostID, $inApproved = 1 ) {
    global $tableNamePrefix;
 ...
 */
function queryDatabase( $inQueryString ) {

    $result = mysql_query( $inQueryString )
        or fatalError( "Database query failed:<BR>$inQueryString<BR><BR>" .
                       mysql_error() );
 ...
 */
function doesTableExist( $inTableName ) {
 ...
 */
function printDateTimeForm( $namePrefix,
                            $fillWithCurrentTime = 0,
 ...
 */
function printDateTimeFormFromTimestamp( $namePrefix,
                                         $fillWithCurrentTime = 0,
                                         $selectedTimestamp ) {

    if( $fillWithCurrentTime ) {
        // ignore selectedTimestamp
        printDateTimeForm( $namePrefix, $fillWithCurrentTime );
        }
    else {
 ...
 */
function formatTime( $year, $month, $day, $hour, $minute, $second, $ampm ) {
 ...
 */
function computePasswordHash( $user_id, $password ) {
 ...
 */
function computeSessionID( $user_id, $password ) {
    global $siteShortName;
 ...
 */
function refreshCookie( $user_id, $session_id ) {
    global $cookieName;

    // expire in 24 hours
    $expireTime = time() + 60 * 60 * 24;
    
    setcookie( $cookieName ."_user_id", stripMagicQuotes( $user_id ),
               $expireTime, "/" );
...
function getLoggedInUser() {
    ...
    if( ! $justLoggedOut &&
        ...
        $trueSessionID = getUserDatabaseField( $cookie_user_id, "session_id" );
...
function isAdministrator() {
    ...
    if( getUserDatabaseField( $loggedInID, "administrator" ) == 1 ) {
 ...
 */
function getUserDatabaseField( $user_id, $fieldName ) {
    global $tableNamePrefix;
 ...
 * @param $fieldName the name of the field to get.
 */
function getPostDatabaseField( $post_id, $fieldName ) {
    global $tableNamePrefix;
 ...
 *   Defaults to 1.
 */
function setUserDatabaseField( $user_id, $fieldName, $fieldValue,
                               $autoQuote = 1 ) {
    global $tableNamePrefix;
 ...
 */
function addslashes_deep( $inValue ) {
    return
        ( is_array( $inValue )
          ? array_map( 'addslashes_deep', $inValue )
          : addslashes( $inValue ) );
    }
 ...
 */
function stripslashes_deep( $inValue ) {
    return
        ( is_array( $inValue )
          ? array_map( 'stripslashes_deep', $inValue )
 ...
 */
function getRequestVariableRaw( $inVariableName ) {
 ...
 */
function getRequestVariableSafe( $inVariableName ) {
...
function getUserCount() {
    ...
    connectToDatabase();
    $result =
        queryDatabase( "SELECT COUNT(*) FROM $tableNamePrefix"."users;" );
...
function canEdit( $inPostID ) {
    ...
    if( strcmp( $loggedInID,
                getPostDatabaseField( $inPostID, "user_id" ) ) == 0 ) {
 ...
 */
function isPostVisible( $inPostID ) {
    global $tableNamePrefix;
    $query =
        "SELECT COUNT(*) " .
        "FROM $tableNamePrefix"."posts " .
...
function rcb_blog2html( $inData ){
    ...
    $replace  = array(
        ...
        '<img border=0 class="left" src="$1" alt="$1" />',
        '<img border=0 class="right" src="$1" alt="$1" />',
        '<img border=0 src="$1" alt="$2" title="$2" />',
        '<img border=0 class="left" src="$1" alt="$2" title="$2" />',
        '<img border=0 class="right" src="$1" alt="$2" title="$2" />',
        '<div style="clear:both"></div>',
			
        // [hr], \n
        '<hr />',
        '<br />');
    return preg_replace($patterns, $replace, rcb_striphtml( $inData ) );
	}
